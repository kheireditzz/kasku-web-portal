      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        {activeTab === 'kasir' ? (
          <div className="flex items-center gap-2.5 select-none">
            <div className="w-8 h-8 rounded-xl overflow-hidden shadow-sm flex items-center justify-center shrink-0 bg-[#0d1117]">
        ...
        {/* Desktop Navigation Links - Shown only when on KasKu tabs */}
        {activeTab !== 'kasir' && (
        ...
        <div className="flex items-center gap-2.5">
          {activeTab !== 'kasir' ? (
            ...
            /* Mode KasirKu: Tombol Pengaturan Ikon Murni (Tanpa Teks) */
            onOpenSettingsModal && (
              <button
                type="button"
                onClick={(e) => {
                ...
                title="Pengaturan KasirKu & Beralih ke KasKu"
                className="w-9 h-9 rounded-full bg-white text-slate-700 border border-slate-200/80 flex items-center justify-center transition-transform duration-150 shadow-xs active:scale-95 group cursor-pointer touch-manipulation select-none"
              >
                <Cog6ToothIcon className="w-4 h-4 text-slate-600 pointer-events-none" />
              </button>
